...
#設定されていなければならない
$CGI::POST_MAX = 1024 * 250; #max = 250kB
...
if($q->param('filetype') eq 'img'){
	my ($sec, $min, $hour, $mday, $mon, $year, $wday)
		= localtime(time);
	my $date = sprintf("%02d/%02d%02d:%02d",++$mon,$mday,$hour,$min);
	$path = "./custom/img";
	$script= qq|window.opener.document.getElementById("boardimg").src="$path/$fname?$date";|; 
	#受付可能な拡張子（正規表現）
	@ext_ok = qw (png);
}
...
print "ファイルをアップロードしました<br>1秒後に閉じます$fh\n\n";
